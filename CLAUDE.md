...
목표: 표본적 탐색의 한계를 로테이션·다각도 검색·워치리스트 승계로 보완해, 업데이트가 반복될수록 편성이 "찾을 수 있는 최선"으로 수렴하게 한다.

1. **우선순위 + 로테이션**: ① 목표가 소진·성과 부진·경고 종목이 속한 (주제×국가)를 최우선 탐색. ② 그 외에 `WATCHLIST.md`의 "주제별 마지막 탐색일"에서 **가장 오래된 (주제×국가) 그룹 최소 2개**를 골라 깊게 탐색하고 탐색일을 갱신한다 — 이렇게 하면 10개 전 그룹이 주기적으로 빠짐없이 커버된다.
...
4. **교체 판정 기준**(모두 충족 시에만 교체): ① 후보가 주제 정의에 부합, ② 신뢰 출처 2개 이상 교차확인, ③ 필수 수치(현재가·목표가 컨센서스·배당수익률)가 전부 스니펫에서 확인됨, ④ 기존 종목 대비 명확히 우월(상승여력·실적 모멘텀이 뚜렷이 크거나, 기존 종목에 결격 — 목표가 소진·논거 훼손 — 존재). 하루 최대 4교체, 우월 후보가 없으면 결과에 '교체 0건'을 명시한다(정상 동작).
