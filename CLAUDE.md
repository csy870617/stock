...
4. **교체 판정 기준**(모두 충족 시에만 교체): ① 후보가 주제 정의에 부합, ② 신뢰 출처 2개 이상 교차확인, ③ 필수 수치(현재가·목표가 컨센서스·배당수익률)가 전부 스니펫에서 확인됨, ④ 기존 종목 대비 명확히 우월(상승여력·실적 모멘텀이 뚜렷이 크거나, 기존 종목에 결격 — 목표가 소진·논거 훼손 — 존재). 하루 최대 10교체, 우월 후보가 없으면 결과에 '교체 0건'을 명시한다(정상 동작).
