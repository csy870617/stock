...
6. **AI 적정가 병기(`aiTarget`·`aiBasis`, 선택)**: 컨센서스 목표가와 별개로, **스니펫에서 검증된 입력값만** 사용해 명시적 공식으로 자체 적정가를 산출·병기할 수 있다 — 이익주는 `검증 EPS × 적정 PER(업종 통상 밴드, 판단 근거 명시)`, 배당주는 `검증 DPS ÷ 요구 배당수익률(밴드·판단 명시)`, 금융주는 PBR-ROE 등. `aiBasis`에 공식·입력값·판단 근거를 반드시 기록한다(validate 가 근거 없는 aiTarget 저장을 거부, 컨센서스 대비 ±50% 괴리는 경고). 입력값을 검증 못 하면 생략이 정상 — 추정치로 채우지 않는다. 종목을 손볼 때마다 aiTarget 커버리지를 점진 확대한다.
